...
const TOPIC_CONTENT: Record<HowItWorksTopic, { title: string; lines: string[] }> = {
  ...
  items: {
    ...
    lines: [
      ...
      "Для типа «Деталь» открывается форма «Новая деталь (Цех)», после сохранения позиция создаётся автоматически.",
      "Позиция кооперации не требует дат: достаточно описания, количества и комментария.",
    ],
  },
  queue: {
    title: "Как работает очередь",
    lines: [
      "Задания создаются из позиций типа «Деталь» и попадают в бэклог.",
      "После назначения станка задание переходит в «Готово к запуску», затем в «В работе» и «Завершено».",
      "Статус «Заблокировано» используется только с причиной блокировки.",
    ],
  },
  access: {
